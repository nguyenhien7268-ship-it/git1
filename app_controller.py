...
# Chúng ta sẽ không import, mà gọi hàm mới bên UI
...
class AppController:
    ...
    # =NOTIZ: Đã thay thế self.update_output bằng self.logger.log
    #         Đã thay thế self.root.after bằng self.root_after
    # ===================================================================
    ...
    def task_run_update_from_text(self, raw_data):
        try:
            ...
            if success:
                # (SỬA LỖI) Gọi hàm mới bên UI, không dùng tk.END
                self.root_after(0, self.app.clear_update_text_area) # Gọi về App
