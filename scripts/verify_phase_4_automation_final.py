...
try:
    ...
    # Import các hàm logic cốt lõi (sử dụng alias để mock)
    from logic.de_backtester_core import calculate_de_bridge_max_lose_history 
    # Note: data_repository imports chỉ để reference, không sử dụng trực tiếp trong test
    # try:
...
MOCK_BRIDGES = [
    ...
    {
        'id': 3, 
        'name': 'LO_RISK', # Cầu Lô không bị ảnh hưởng bởi Pruning Đề
        'is_enabled': 1, 
        'is_pinned': 0, 
        'type': 'LO_FIXED_STL'
    },
...
class TestPhase4Automation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 2. Mocking DataRepository (Giả lập hàm lấy cầu)
        ...
        # Clone list để mỗi test có thể thay đổi thuộc tính is_pinned
        self.mock_get_all_bridges.return_value = [b.copy() for b in MOCK_BRIDGES] 
    ...
    def test_02_prune_bad_de_bridges_protection(self):
        ...
        # Cấu hình Pruning Logic: Báo 50 ngày gãy (MaxLose=50 > Ngưỡng=30)
        self.mock_max_lose_history.return_value = 50 

        # 1. Giả lập cầu GOOD được GHIM (is_pinned=1) và cầu RISK chưa ghim (is_pinned=0)
        mocked_data = self.mock_get_all_bridges.return_value
        mocked_data[0]['is_pinned'] = 1 # Cầu GOOD được ghim
        mocked_data[1]['is_pinned'] = 0 # Cầu RISK không ghim
    ...
    def test_03_pruning_logic_is_active(self):
        ...
        # Đảm bảo cả hai cầu Đề đều không ghim
        mocked_data = self.mock_get_all_bridges.return_value
        mocked_data[0]['is_pinned'] = 0 
        mocked_data[1]['is_pinned'] = 0 
        mocked_data[2]['type'] = 'DE_POS_TOUCH' # Đảm bảo cầu thứ 3 cũng là cầu Đề để test Pruning
        ...
        # Hàm update_managed_bridge phải được gọi ít nhất 2 lần (cho 2 cầu)
