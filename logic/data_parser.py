...
#
# (NỘI DUNG THAY THẾ TOÀN BỘ - MERGE V7 (JSON Detect) + V6 (Lotto Parse & DB Schema))
#
import json
import re
import traceback
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    print(f"Bỏ qua kỳ {ky_str}: Định dạng ngày không hợp lệ '{date_str}'")
                    return None
        ...
        if isinstance(ky_data, list) and len(ky_data) == 8:
            ...
            if isinstance(ky_data[0], list) and len(ky_data[0]) >= 2:
                ...
                prize_map = {
                    "đặc biệt": "gdb", "nhất": "g1", "nhì": "g2", "ba": "g3",
                    "bốn": "g4", "năm": "g5", "sáu": "g6", "bảy": "g7"
                }
                temp_dict = {}
        ...
        giai_values_for_db = [
            ",".join(gdb_nums), ",".join(g1_nums), ",".join(g2_nums),
            ",".join(g3_nums), ",".join(g4_nums), ",".join(g5_nums),
            ",".join(g6_nums), ",".join(g7_nums)
        ]
...
def _insert_data_batch(cursor, data_list):
    ...
    dulieu_ai_batch = [
        (
            row[0], # MaSoKy (ky)
            row[0], # Col_A_Ky (ky)
            row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[9] # gdb (Col_B) -> g7 (Col_I)
        )
    ...
    try:
        ...
        # (SỬA LỖI V2) THỬ 2: ĐỊNH DẠNG WEB JSON ({"kyInfo": [...], "tablesData": [...]})
            ...
            if len(ky_info_list) * 2 != len(tables_data_list):
                print(f"Lỗi: JSON (Web) không khớp. kyInfo ({len(ky_info_list)}) và tablesData ({len(tables_data_list)}) không theo tỷ lệ 1:2.")
                ...
                for i in range(len(ky_info_list)):
                    ...
                    if ky_str_check and ky_str_check.isdigit():
                         ...
                         combined_list.append((int(ky_str_check), ky_info_list[i], tables_data_list[i*2])) 
                ...
                for ky_int, ky_info, table_data in combined_list:
                    ...
                    content = table_data.get("content", []) # [["Đặc Biệt", "33963"], ...]
                    
                    # <<< BỘ CHUYỂN ĐỔI (ADAPTER) >>>
                    # Chuyển [["ĐB", "123"], ...] THÀNH ["123", "456", ...]
                    giai_data_list = []
                    # (SỬA LỖI V2) Kiểm tra kỹ đây là bảng giải (có "Đặc Biệt")
                    if (isinstance(content, list) and len(content) == 8 and 
                        isinstance(content[0], list) and len(content[0]) >= 2 and 
                        ("Đặc Biệt" in content[0][0] or "GDB" in content[0][0])):
                        
                        for giai_pair in content:
                            giai_data_list.append(giai_pair[1]) # Chỉ lấy chuỗi số (ví dụ: "13173 -23763")
                        ...
                        print(f"Bỏ qua kỳ {ky_str}: Lỗi định dạng (Web), không phải bảng giải thưởng (index chẵn).")
...
def parse_and_APPEND_data(raw_data, conn, cursor):
    ...
    """
    latest_ky_str, latest_date_obj = get_latest_ky_date(cursor) # (Sửa) Lấy từ V6
    ...
    try:
        ...
        # (SỬA LỖI V2) THỬ 2: ĐỊNH DẠNG WEB JSON ({"kyInfo": [...], "tablesData": [...]})
            ...
            # (SỬA V3) Nếu không phải 2 dạng trên, thử dạng Text V6
            print("(V7.0 - Append) Không phải JSON V7/Web. Đang thử định dạng Text (V6)...")
            total_inserted = parse_and_APPEND_data_TEXT(raw_data, conn, cursor)
            conn.commit()
            return total_inserted
...
def parse_and_APPEND_data_TEXT(raw_data, conn, cursor):
    ...
    (Sử dụng lại logic V6 từ Turn 6)
    """
    parsed_data = []
    current_ky_str = None
    ...
    latest_ky, latest_date = get_latest_ky_date(cursor) # (Sửa V7) dùng cursor
    latest_ky_int = 0
    ...
    for line in lines:
        ...
        if match_found:
            ...
            # Tối ưu: Bỏ qua nếu kỳ này cũ
            try:
                if int(current_ky_str) <= latest_ky_int:
        ...
        # Phát hiện dòng GĐB (phải có GĐB trước)
        if ("ĐB" in line.upper() or "GDB" in line.upper()) and current_ky_str:
            giai_data = line.split(":")[-1].strip()
            if giai_data:
                current_ky_data = [giai_data] # Không xóa space/dash ở đây, để _parse_single_ky xử lý
            continue
        
        # Các dòng giải còn lại (G1, G2...)
        elif re.match(r"^G[1-7]", line, re.IGNORECASE) and current_ky_str:
            giai_data = line.split(":")[-1].strip()
            if giai_data and len(current_ky_data) < 8:
                current_ky_data.append(giai_data)

        # Các dòng giải không có chữ G (chỉ có số)
        elif re.match(r"^[ \d,-]+$", line) and current_ky_str:
            giai_data = line.strip()
            if giai_data and len(current_ky_data) < 8:
                 current_ky_data.append(giai_data)
        
        # (Sửa V7) Hỗ trợ định dạng text V7 (các giải dính liền)
...
def run_and_update_from_text(raw_data):
    ...
    try:
        ...
        if conn:
            conn.close()
        return False, f"Lỗi nghiêm trọng khi thêm từ text: {e}\n{traceback.format_exc()}"