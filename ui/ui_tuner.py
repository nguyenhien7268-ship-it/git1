...
class TunerWindow:
    ...
    def __init__(self, app):
        ...
        # --- Khung Cài đặt ---
        settings_frame = ttk.Labelframe(main_frame, text="1. Chọn Tham số để Kiểm thử", padding="10")
        ...
        ttk.Label(settings_frame, text="Chọn tham số:").grid(row=0, column=0, sticky="w", padx=5, pady=5)
        self.param_var = tk.StringVar()
        param_dropdown = ttk.Combobox(settings_frame, 
        ...
        ttk.Label(range_frame, text="Bước nhảy:").grid(row=0, column=4, sticky="w", padx=5)
        ...
        # --- Nút Chạy ---
        self.run_button = ttk.Button(main_frame, text="Chạy Phân tích Tinh chỉnh", command=self.run_tuning)
        self.run_button.grid(row=1, column=0, sticky="ew", padx=10, pady=10)
        
        # --- Khung Log Kết quả ---
        log_frame = ttk.Labelframe(main_frame, text="2. Kết quả Phân tích", padding="10")
    ...
    def run_tuning(self):
        ...
        try:
            # 1. Lấy tham số
            selected_name = self.param_var.get()
            param_key = next((key for key, value in self.tunable_parameters.items() if value == selected_name), None)
            if not param_key:
                messagebox.showerror("Lỗi", "Vui lòng chọn một tham số.", parent=self.window)
                return
            ...
            if val_step <= 0:
                messagebox.showerror("Lỗi", "Bước nhảy phải lớn hơn 0.", parent=self.window)
                return
            if val_from > val_to:
                 messagebox.showerror("Lỗi", "Giá trị 'Từ' không thể lớn hơn giá trị 'Đến'.", parent=self.window)
                 return
            ...
            self.log(f"Khoảng kiểm thử: Từ {val_from} đến {val_to}, bước nhảy {val_step}")
